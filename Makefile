...
CTQMC_gx : $(BASE_OBJS) $(MAIN_OBJS) $(EVALSIM_OBJS) $(HOST_OBJS) $(GPU_OBJS) $(GPU_CUDA_OBJS)
	@mkdir -p $(EXE_DIR)
	$(CXX_MPI) $(CPPFLAGS) $(CXXFLAGS) $(BASE_OBJS) $(MAIN_OBJS) $(EVALSIM_OBJS) $(HOST_OBJS) $(GPU_OBJS) $(GPU_CUDA_OBJS) -o $(EXE_DIR)$@ $(GPU_DIR)Main.C $(LDFLAGS) $(LIBS)

$(GPU_CUDA_OBJS) :
	$(NVCC) $(NVCCFLAGS) $(CUDA_CPPFLAGS) $(CUTLASS_CPPFLAGS) -dc $(@:.o=.cu) -o $@bj.o
	$(NVCC) -arch=$(GPU_ARCH) -dlink $@bj.o -o $@
