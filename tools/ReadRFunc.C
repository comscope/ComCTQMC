...
int main(int argc, char** argv)
{
	try {		
        ...
        std::size_t min = jsx::at<io::rvec>(jObservables(name)(entries[0])("function")).size();
        for(std::size_t i = 0; i < entries.size(); ++i)
            min = std::min(min, jsx::at<io::rvec>(jObservables(name)(entries[i])("function")).size());
        ...
        for(std::size_t n = 0; n < min; ++n) {
            file << 2*n*M_PI/beta;
            for(std::size_t i = 0; i < entries.size(); ++i)
                file << " " << jsx::at<io::rvec>(jObservables(name)(entries[i])("function"))[n];
            file << std::endl;
        }
