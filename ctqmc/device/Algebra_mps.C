...
void imp::init_device(std::vector<char> const& pciId, std::size_t) {
    ...
    cudaErrchk(cudaMalloc(reinterpret_cast<void**>(&tempDevice), sizeof(double)));
    tempHost = new double; /// ?????????????????
}
...
void imp::release_device()
{
    ...
    cudaErrchk(cudaFree(tempDevice));
    delete tempHost;
};
...
imp::Matrix<Device, double>::Matrix(int size) : data_(size) {
}
imp::Matrix<Device, double>::Matrix(Matrix<Device, double>::Identity const& identity) : I_(identity.dim), J_(identity.dim), data_(I_*J_), exponent_(.0) {
...
imp::Matrix<Device, double>::Matrix(Matrix<Device, double>::Zero const& zero) : I_(zero.dim), J_(zero.dim), data_(I_*J_), exponent_(.0) {
    double* temp = new double[I_*J_]; std::memset(temp, 0, I_*J_*sizeof(double));
...
imp::Matrix<Device, double>::Matrix(int I, int J, io::rmat const& mat) : I_(I), J_(J), data_(I_*J_), exponent_(.0) {
...
void imp::mult(Matrix<Device, double>& dest, Matrix<Device, double> const& L, Matrix<Device, double> const& R, itf::Batcher<double>& batcher) {
    ...
    cudaErrchk(cutlassDgemm(R.J(), L.I(), L.J(), R.data().ptr(), R.J(), L.data().ptr(), L.J(), dest.data().ptr(), R.J()));
...
void imp::trace(ut::Zahl<double>* Z, ut::Zahl<double>* accZ, Matrix<Device, double> const& matrix, itf::Batcher<double>& batcher) {
    ...
    args.result = tempDevice;
    args.dim    = matrix.I();
    
    kerTrace<WarpSize><<<1, WarpSize>>>(args);
    
    cudaErrchk(cudaMemcpy(tempHost, tempDevice, sizeof(double), cudaMemcpyDeviceToHost));
    
    ut::Zahl<double> temp(*tempHost, matrix.exponent());
    if(Z) *Z = temp; if(accZ) *accZ += temp;
}
...
void imp::traceAtB(ut::Zahl<double>* Z, ut::Zahl<double>* accZ, Matrix<Device, double> const& At, Matrix<Device, double> const& B, itf::Batcher<double>& batcher) {
    ...
    ut::Zahl<double> temp(*tempHost, At.exponent() + B.exponent());
    if(Z) *Z = temp; if(accZ) *accZ += temp;
}
...
void imp::norm(double* norm, Matrix<Device, double> const& matrix, itf::Batcher<double>& batcher) {
    ...
    *norm = std::log(*tempHost)/2. + matrix.exponent();
}
