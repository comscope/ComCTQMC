...
inline
double get_closest_lambda(double lambda){
    std::vector<double> database_lambdas{10,100,1000,10000};
    auto const it_l = std::lower_bound(database_lambdas.begin(), database_lambdas.end(), lambda);
    auto const it_u = std::upper_bound(database_lambdas.begin(), database_lambdas.end(), lambda);
    
    if (it_l == database_lambdas.end())
        lambda = database_lambdas.back();
    else if (std::abs(*it_l - lambda) > std::abs(*it_u - lambda))
        lambda = *it_l;
    else
        lambda = *it_u;
